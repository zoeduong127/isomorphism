...
Class Vertex: Vertex of a graph (graph, label, incidence), has the following functions that can be used:
              1. is_adjacent;
              2. _add_incidence.
Class Edge: Edge of a graph (tail, head, weight), has the following functions that can be used:
            1. other_end;
            2. incident.
Class Graph: The graph itself (list of vertices, list of edges, simple, directed), has the following functions:
             1. add_vertex;
             2. add_edge;
             3. find_edge;
             4. is_adjacent.
...
class Vertex(object):
    ...
    def __init__(self, graph: "Graph", label=None):
        ...
        self.label = label
        self._incidence = {}
        self.colornum = self.label      # Comment this if you don't need coloured graphs
    ...
    def add_edge(self, edge: "Edge"):
        ...
        if self._simple:
            ...
            if self.is_adjacent(edge.tail, edge.head):
                raise GraphError('No multi edges allowed in simple graphs')
